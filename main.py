...
class Message(discord.ui.View):
    ...
    @discord.ui.button(label="Sim, confirmar!", emoji=discord.PartialEmoji.from_str("<a:870209867221200906:>"), style=discord.ButtonStyle.success)
    async def iniciar(self, interaction: discord.Interaction, button: discord.ui.Button, custom_id='persistent_view:green'):
        await interaction.response.send_modal(InputModal(interaction))

    @discord.ui.button(label="Não, cancelar.", emoji=discord.PartialEmoji.from_str("<a:870209867221200906:>"), style=discord.ButtonStyle.red)
    async def finalizar(self, interaction: discord.Interaction, button: discord.ui.Button, custom_id='persistent_view:red'):
...
class InputModal(ui.Modal, title="Postagem com Embed"):
    ...
    custom_color = ui.TextInput(label="Escolha a cor:", placeholder="Apenas roxo ou rosa", style=discord.TextStyle.short,
                              required=False)
